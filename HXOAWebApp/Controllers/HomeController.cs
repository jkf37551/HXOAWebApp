    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult Login(FormCollection collection)
        {
            ...
            try
            {
                ...
                HRMDBDataContext DB = new HRMDBDataContext();
                var loginUser = DB.Users.Where(p => p.UserId.Equals(UserId)).FirstOrDefault();
                if (loginUser == null)
                    loginUser = new Users();
                Session[ConstDictionary.LoginUser] = loginUser;
        ...
        public ActionResult MainDefault()
        {
            try
            {
                Users loginUser = (Users)Session[ConstDictionary.LoginUser];
                M_SYS_INFO info = new M_SYS_INFO();
                info.SYS_UPDATE_CONTENT = GetTree("SYS", loginUser);
                ViewBag.MenuTrees = info.SYS_UPDATE_CONTENT;
                return View();
            }
            catch (Exception ex)
            {
        ...
        private string GetTree(string ID, Users loginUser)
        {
            ...
            try
            {
                HXWebDBDataContext DB = new HXWebDBDataContext();
                ...
                if (items != null && items.Count > 0)
                {
                    foreach (var item in items)
                    {
                        if (item.MENU_CODE.Equals("MAIN_BUSY")) //默认选中系统业务
                            TreeStr = TreeStr + "<div title=\"" + item.MENU_NAME + "\" iconCls=\"icon-light\" selected=\"true\" style=\"overflow:auto;padding:2px;\">\n";
                        else
                            TreeStr = TreeStr + "<div title=\"" + item.MENU_NAME + "\" iconCls=\"icon-light\" selected=\"false\" style=\"overflow:auto;padding:2px;\">\n";
